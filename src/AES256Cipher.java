...
public class AES256Cipher
    ...
    // misc settings
    private static String ALGO_TRANSFORMATION_STRING = "AES/CBC/PKCS5Padding"; // AES algorithm, with CBC mode, and
    ...
    private static String ADDITIONAL_AUTHENTICATED_DATA = "AES256Cipher"; // this plain text is packaged with the
                                                                          // encrypted data and authenticated
    ...
    /**
     * Encrypt the message bytes with the secret password and return the result as a
     * serialized
    ...
    /**
     * Decrypt the given encrypted bytes contained in the serialized Properties
     * object using the given password.
     * If an error occurs during decryption, the error is printed and null is
     * returned.
     *
     * @param password             secret password to use for decryption
     * @param serializedProperties serialized Properties object which contains all
     *                             expected
    ...
    public static byte[] decrypt(char[] password, byte[] serializedProperties) {
        ...
        try {
            ...
            ByteArrayInputStream bytinpstm = new ByteArrayInputStream(serializedProperties, RANDOM_HEADER_BYTE_LENGTH,
                    serializedProperties.length - RANDOM_HEADER_BYTE_LENGTH);
            Properties prp = new Properties();
            prp.load(bytinpstm);
    ...
    private static SecretKey deriveSecretKey(char[] password, byte[] salt) throws Exception {
        ...
        return new SecretKeySpec(pbkdfKeyFactory.generateSecret(keySpec).getEncoded(), "AES"); // associate secret-key
                                                                                               // algorithm
    }
